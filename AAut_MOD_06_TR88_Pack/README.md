...
<figure>
  ...
  <figcaption>Machine Mikron G05</figcaption>
</figure>
...
<figure>
    <img src="./img/Bekhoff PackMLFullStateMachine.png"
         alt="Image of Bekhoff PackMLFullStateMachine.png">
...
###	Clearing 
State Type: <span style="color:red; font-weight:bold">Acting</span>
...
Command	N° PackML	Implemented on Unit
        External interface	Soft push Button	Hard Push Button	Name of button
...
2	MAINTENANCE	Optional	Yes	Utilisé pour la maintenance préventive de routine ou la maintenance planifiée. Doit être utilisé pour documenter l'occurrence et la durée de la maintenance planifiée.
Ce mode permet au personnel autorisé de faire fonctionner l'unité / la machine indépendamment des autres systèmes. Ce mode est généralement utilisé pour la recherche de pannes, les essais de machines ou les tests d'améliorations opérationnelles.
Par exemple : Le nettoyage d'une tête d'impression est de la maintenance.
3	MANUAL	Optional	Yes	Utilisé pour le diagnostic des pannes d'une intervention technique imprévue.
Remarque : en mode de commande manuelle, il n'est pas nécessaire d’implémenter tous les états PackML.
...
…	?	?	?	?
Figure 36 PackML example of modes
Remarque : Le changement de mode n'a lieu que lorsque l'unité / la machine est à l'état STOPPED. Cela signifie que les modes sortie et entrée sont à l'état STOPPED. Le changement de mode n'apportera aucune modification à l'état PackML. Par conséquent, une unité / machine en mode production et à l'état arrêté passera par exemple en mode maintenance à l'état STOPPED. Le changement de mode peut également se produire à l'état ABORTED, lors du passage en mode manuel. La sortie vers le mode manuel, est soumise à des restrictions en raison de contraintes de sécurité.
...
Figure 37 PackML State Model Maintenance
###	Mode manuel
...
Figure 38 PackML State Model Manual
###	Timing
...
###	Introduction
Les PackTags fournissent un ensemble uniforme de conventions de dénomination pour les éléments de données utilisés dans les éléments procéduraux du modèle d'état de base. Comme vu précédemment dans le document, le modèle d'état de base fournit un ensemble uniforme d'états de la machine, de sorte que toutes les machines automatisées puissent être vues de manière commune. Les PackTags sont des éléments de données nommés utilisés pour une architecture ouverte, l'échange de données interopérable dans des machines automatisées.
