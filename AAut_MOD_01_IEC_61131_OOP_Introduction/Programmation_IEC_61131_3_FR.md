...
# Module en français
Le cours de base est maintenant en anglais. Par contre, comme **cette version était disponible en français, elle est conservée dans sa forme originale mais ne fait plus partie officiement du cours.**
...
Il est conseillé de lire le document « PLCopen Software Creation Guidelines : [Creating PLCopen Compliant Libraries V1_0](https://www.google.com/url?sa=t&source=web&rct=j&opi=89978449&url=https://www.plcopen.org/download_file/view/7ea712b1-1f89-43fc-870c-6006e415956f/&ved=2ahUKEwjKurmAn96SAxXW_wIHHV5kDtoQFnoECBsQAQ&usg=AOvVaw1vms_RuVDGuJ5-5YvPcxbP), version 1.0 du 4 mai 2017 », téléchargeable sur le site web de PLCopen. Dans ce cadre, nous utiliserons les principes **SOLID** dans l’environnement des technologies d’automatisation. SOLID signifie :
...
- **ISP — Interface Segregation Principle**: Il vaut mieux avoir plusieurs interfaces spécifiques plutôt qu'une seule interface générale.
...
**Conseil**: pour les variables de type VAR_IN_OUT qui doivent être utilisées dans une méthode, passer par une variable interne intermédiaire identifiée par un _underscore.
...
- Vous pouvez ensuite assigner la variable d'interface à chaque instance d'un bloc fonctionnel qui implémente correctement l'interface. Si aucune variable d'interface n'a encore été assignée, la variable contient la valeur 0 en mode en ligne.
...
- Une fonction ``F_DeliverType`` contient la déclaration d'une variable d'entrée du type d'interface ``I_Sample``. Dans la fonction, la méthode d'interface GetType est appelée via la variable d'interface ``iSample``. Dans ce cas, le fait que ``FB_A.GetType`` ou ``FB_B.GetType`` soit appelé dépend du type de bloc de fonctions transféré, voir: [Appel de méthodes et de propriétés d'une instance de bloc fonctionnel via une variable d'interface](#appel-de-méthodes-et-de-propriétés-dune-instance-de-bloc-fonctionnel-via-une-variable-dinterface).
